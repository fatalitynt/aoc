...
public class Day9
{
    public long SolvePart1()
    {
        ...
        return P.GetAllPoints(a)
            .Where(p => p.Get4ValidNeighbors(a).All(x => p.Read(a) < x.Read(a)))
            .Select(x => x.Read(a))
    ...
    private long GetPool(P s, int[][] a)
    {
        ...
        while (q.Count > 0)
        {
            var cur = q.Dequeue();
            var cVal = cur.Read(a);
            foreach (var neigh in cur.Get4ValidNeighbors(a))
            {
    ...
    public long SolvePart2()
    {
        ...
        var lowP = P.GetAllPoints(a)
            .Where(p => p.Get4ValidNeighbors(a).All(x => p.Read(a) < x.Read(a))).ToArray();
